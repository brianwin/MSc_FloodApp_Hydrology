create table if not exists production.station_scale
(
    station_scale_id         serial
        primary key,
    created                  timestamp default CURRENT_TIMESTAMP,
    station_id               integer
        constraint fk_station_scale_station_id
            references station
            on delete cascade,
...
alter table station_scale
    owner to wmon;

create index if not exists idx_station_scale_station_id
    on station_scale (station_id);
